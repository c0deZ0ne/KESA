(function ($) {
  ...
  if ($(".deals-sliders").length > 0) {
    ...
    owl.owlCarousel({
      ...
      responsive: { 0: { items: 1 }, 991: { items: 2 }, 1170: { items: 2 } },
    });
  }
  ...
  if ($("#weight-status").length > 0) {
    var options = {
      series: [
        { name: "Weight", data: [34, 44, 54, 21, 12, 43, 33, 23, 66, 66, 58] },
      ],
  ...
  $("a.smooth-menu").on("click", function (e) {
    ...
    $("html, body")
      .stop()
      .animate({ scrollTop: $($anchor.attr("href")).offset().top });
    e.preventDefault();
  });
  ...
  $(function () {
    var $grid = $(".grid").isotope({ itemSelector: "article" });
